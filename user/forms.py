...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        # 登录认证，验证用户名和密码是否匹配，如果不匹配返回 None
...
class RegForm(forms.Form):
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username=username).exists(): # 验证用户名是否存在
            raise forms.ValidationError('用户名已存在')
        return username

    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.filter(email=email).exists(): # 验证邮箱是否存在
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise form.ValidationError('你已经绑定邮箱')
...
class ForgotPasswordForm(forms.Form):
    ...
    verification_code = forms.CharField(
        label='验证码', 
        required=False,
        widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'请输入旧的验证码'}))
