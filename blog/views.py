...
from django.db.models import Count #导入计数方法，供annotate使用
...
def get_blog_list_common_data(request, blogs_all_list):
    '''获取博客列表公共数据'''
    paginator = Paginator(blogs_all_list, settings.EACH_PAGE_BLOGS_NUMBER)# 用分页器分页，每页显示几条博客
    page_num = request.GET.get('page', 1)# 获取请求中的页码参数
    page_of_blogs = paginator.get_page(page_num) # 某一页的所有博客
    ...
    context['page_of_blogs'] = page_of_blogs # 某一页的所有博客传到前端页面
